...
> **Status: OFFLINE as of 2026-05-30.** minerva has been decommissioned. This file reflects the
> last known live configuration. See `docs/hibernation.md` for revival instructions.
...
- **minerva** — MacBook Pro, Apple Silicon. Last active host. OpenClaw gateway at `127.0.0.1:18789` (loopback only), managed by launchd (`ai.openclaw.gateway`). Chroma vector store at `127.0.0.1:8000` (`ai.cicero.chroma`). **Decommissioned 2026-05-30.**
...
- **Apple Reminders** *(Mac-only — unavailable on Linux):* Three shared lists owned by Carlos, all shared with `cicero.ortega@icloud.com`:
